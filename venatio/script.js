...
function getPieceOwner(pieceType) {
  if (pieceType === HUNTER) {
    return hunterOwner;
  }
...
function getPlayerColor(playerNumber) {
  return playerNumber === "1" ? "wit" : "zwart";
}
...
function updateStatus() {
  ...
  if (!selected) {
    turnText.textContent =
      `${currentPlayer === HUNTER ? "Jager" : "Prooi"} (${color}) is aan zet`;
  } else {
    turnText.textContent =
...
function createPiece(player, index) {
  ...
  const owner = getPieceOwner(player);
